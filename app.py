...
def enviar_emails(aluno_email, nome_aluno, nota, relato, arquivos):
    ...
    try:
        # Loop para enviar para Professor e Aluno
...
if st.session_state.etapa == 'login':
    ...
    n = st.text_input("Nome Completo", key="input_nome")
    t = st.text_input("Turma", key="input_turma").strip().upper()
    e = st.text_input("E-mail", key="input_email")
    ...
    st.title("Laboratório de Exercícios")
    st.write(f"Candidato: **{st.session_state.aluno['nome']}**")
    
    st.subheader("1. Baixar Desafio")
    st.download_button("📥 Download Planilha", st.session_state.prova_origem, f"Prova_{st.session_state.aluno['nome']}.xlsx")
    
    st.divider()
    
    st.subheader("2. Enviar Resolução")
    ups = st.file_uploader("Upload (xlsx/xlsm)", type=['xlsx', 'xlsm'], accept_multiple_files=True)
    
    if st.button("🚀 Finalizar e Enviar"):
        if ups:
            nota, relato = realizar_correcao_completa(ups)
            st.session_state.resultado_final = {"nota": nota, "relato": relato}
            
            sucesso = enviar_emails(st.session_state.aluno['email'], st.session_state.aluno['nome'], nota, relato, ups)
            
            if sucesso:
                st.success(f"Nota: {nota}/100. Relatório enviado para o seu e-mail e para o professor!")
                st.text(relato)
            else:
                st.warning("Nota calculada, mas houve um erro no envio dos e-mails.")

    st.divider()
    if st.button("🛑 Encerrar Sessão (Limpar Tudo)"):
        encerrar_sessao()

# --- DASHBOARD ADM (Mantido) ---
with st.expander("👤 Área do Professor"):
    m_senha = st.text_input("Senha Mestra", type="password")
    if m_senha == "ricardoitmaster":
        if os.path.exists("db_notas.csv"):
            st.dataframe(pd.read_csv("db_notas.csv"))
